...
```css {{ filename: 'select2-theme.css' }}
.select2-dropdown {
...
```html {{ example: { p: 'none', hint: 'Scroll this table to see the sticky header row in action' } }}
  ...
  <form>
    <div>
      ...
      <div class="mt-1 relative">
        <input required type="text" name="email" id="optional-email" class="peer px-3 py-2 bg-white border shadow-sm border-slate-300 placeholder-slate-400 focus:outline-none focus:border-sky-500 focus:ring-sky-500 block w-full rounded-md ~/sm:~text-base/sm focus:ring-1" />
    ...
    <div class="mt-6">
      ...
      <div class="mt-1 relative">
        <input type="text" name="name" id="optional-name" class="peer px-3 py-2 bg-white border shadow-sm border-slate-300 placeholder-slate-400  focus:outline-none focus:border-sky-500 focus:ring-sky-500 block w-full rounded-md ~/sm:~text-base/sm focus:ring-1" />
        <div class="peer-optional:hidden absolute inset-y-0 right-0 text-sm pr-3 py-2 border-y border-transparent text-slate-400">
          Required
        </div>
      </div>
...
Okay so this one is the real highlight for me — you know how we give you the [`addVariant` API](/docs/plugins#adding-variants) for creating your own custom variants?
...
...and you know how we have [arbitrary values](/docs/adding-custom-styles#using-arbitrary-values) for using any value you want with a utility directly in your HTML?
...
```html {{ example: { p: 'none', hint: 'Try hovering over the text &ldquo;Floyd Miles&rdquo;' } }}
<div class="px-4 py-6">
  <div class="max-w-md mx-auto">
